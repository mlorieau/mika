...
<head>
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .adm-nav-section {
    ...
    padding: 0 6px; cursor: default; white-space: nowrap;
    border-left: 1px solid rgba(255,255,255,.08); margin-left: 4px;
  }
  .adm-nav-section:first-child { border-left: none; margin-left: 0; }
  .adm-nav-section.has-active { color: rgba(234,86,73,.6); }
  .adm-topright {
...
<header class="adm-topbar">
  ...
  <nav class="adm-topnav">
    <a href="dashboard.php" <?= ($admin_current ?? '') === 'dashboard' ? 'class="active"' : '' ?> title="Dashboard">⊞</a>
    <?php
    // Sections groupées V12
    $adm_sections = [
      'CONTENUS' => [
    ...
    $cur = $admin_current ?? '';
    foreach ($adm_sections as $section_label => $items):
      $has_active = false;
      foreach ($items as [$f,$slug,$label]) { if ($cur === $slug) { $has_active = true; break; } }
    ?>
    <span class="adm-nav-section <?= $has_active ? 'has-active' : '' ?>"><?= $section_label ?></span>
    <?php foreach ($items as [$file,$slug,$label]): ?>
    <a href="<?= $file ?>" <?= $cur === $slug ? 'class="active"' : '' ?>><?= $label ?></a>
    <?php endforeach; ?>
    <?php endforeach; ?>
  </nav>
